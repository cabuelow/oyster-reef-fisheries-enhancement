...
mod_enhance <- function(dens, spp, mf, m, t_max, t_0, t_harv, l_asym, Ks, a, b, years){
  df <- data.frame(species = spp, male_female = mf, year = 0, d_ind_100m2 = 0, length_cm = 0, weight_g = 0, biomass_g_100m2 = 0, gross_biomass_g_100m2 = 0, weight_g_i = 0, biomass_g_100m2_i = 0, net_biomass_g_100m2 = 0)
  for(i in 1:years){
    df[i+1,1] <- spp # spp
    ...
    df[i+1,8] <- if(i==t_harv){df[i+1,7]}else if(i+1>t_harv){df[i,8]+df[i+1,7]}else(0) # cumulative biomass gains over time (gross gains)
