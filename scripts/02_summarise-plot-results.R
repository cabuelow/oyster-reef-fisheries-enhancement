...
dat2 <- dat %>% 
  ...
  summarise(net_biomass_g_ha = sum(net_biomass_g_ha)) %>%  # here summing by gender 
  group_by(species, year) %>% 
  summarise(net_biomass_g_ha_mean = mean(net_biomass_g_ha, na.rm = T),
...
a <- dat2 %>% 
  group_by(year) %>% 
  summarise(net_enhance = sum(net_biomass_g_ha_mean), 
...
b <- dat2 %>% 
  ggplot() +
  geom_ribbon(aes(x = year, ymin = net_biomass_g_ha_mean - net_biomass_g_ha_sd, 
                  ymax = net_biomass_g_ha_mean + net_biomass_g_ha_sd, group = species), fill = "grey", alpha = 0.5) +
  geom_line(aes(x = year, y = net_biomass_g_ha_mean, col = species)) +
  ylab('') +
  xlab('Year') +
...
c <- dat2 %>% 
  ggplot() +
  geom_area(aes(x = year, y = net_biomass_g_ha_mean, fill = factor(species)), position = 'stack') +
  ylab('') +
  xlab('Year') +
...
e <- dat %>% 
  ...
  summarise(net_biomass_g_ha = sum(net_biomass_g_ha)) %>% # sum by gender
  ggplot() +
  aes(x = m, y = net_biomass_g_ha) +
